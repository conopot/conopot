...
class _InviteScreenState extends State<InviteScreen> {
  ...
  getUserInviteStatus() async {
    ...
    String url = 'http://10.0.2.2:3000/user/invite/status?userId=$userId';
  ...
  getUserInvitePersonCount() async {
    ...
    String url = 'http://10.0.2.2:3000/user/invite/count?userId=$userId';
  ...
  void initState() {
    ...
    //사용자 초대 코드
    userInviteCode =
        Provider.of<NoteData>(context, listen: false).userId.toString() +
            "Ksc9a" +
            Provider.of<NoteData>(context, listen: false).userId.toString();
  ...
  inviteValidation(String inviteCode) {
    ...
    int leftNum = int.parse(inviteCode.substring(0, hashIndex - 1));
    int rightNum =
        int.parse(inviteCode.substring(hashIndex + 1, inviteCode.length - 1));
